...
#makes an array for all the angles
...
def obstacle_pixel(sensor_reading,angle, pose_x,pose_y,pose_theta):
   
   x_coord = math.sin(angle)*sensor_reading
   y_coord = math.cos(angle)*sensor_reading
   
   return convertToPixel(x_coord+pose_x, y_coord+pose_y)
...
while robot.step(SIM_TIMESTEP) != -1:     
    ...
    converted_pose = convertToPixel(pose_x, pose_y)
    positions.append((converted_pose[0], converted_pose[1]))  
    ...
    for sensor_reading, angle in zip(lidar_sensor_readings, lidar_angles):
        obstacle_x, obstacle_y = obstacle_pixel(sensor_reading, angle+pose_theta, pose_x,pose_y,pose_theta)
    ...
    for obstacle_x, obstacle_y in obstacles: 
        # #set color to white for free space
        display.setColor(0xFFFFFF)
        display.drawLine(converted_pose[0], converted_pose[1], obstacle_x,obstacle_y)
        
        
        #set color to blue for obstacles
        display.setColor(0x0000FF)
        display.drawPixel(obstacle_x, obstacle_y)
